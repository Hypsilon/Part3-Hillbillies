...
public class Unit {	
	 ...
	 * @effect	When the unit is following another unit and is currently doing nothing, start moving towards the unit.
	 * @effect 	When the unit is doing nothing and does not have a goal, set the behavior to default behavior.	
	 * @effect  If this unit is executing a task and an activity (statement) is completed execute the next activity.
	 * 
	 * @throws 	ModelException
	...
	public void advanceTime(double deltaT) throws ModelException{
		...
		if (state == State.FALLING) {
			...
			else if (this.getState() == State.NOTHING && followedUnit != null) {
				int[] unitPos = followedUnit.getPosition().toIntArray();
